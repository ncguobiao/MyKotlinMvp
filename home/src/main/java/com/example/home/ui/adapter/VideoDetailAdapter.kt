...
import com.example.baselibrary.glide.GlideRoundTransform
...
class VideoDetailAdapter(context: Context, data: ArrayList<HomeBean.Issue.Item>)
    ...
    init {
        textTypeface = Typeface.createFromAsset(BaseApplication.mContext.assets, "fonts/FZLanTingHeiS-L-GB-Regular.TTF")
    }
    ...
    private fun setVideoDetailInfo(data: HomeBean.Issue.Item, holder: ViewHolder) {
        ...
        with(holder) {
            getView<TextView>(R.id.tv_action_favorites).setOnClickListener {
                Toast.makeText(BaseApplication.mContext, "喜欢", Toast.LENGTH_SHORT).show()
            }
            getView<TextView>(R.id.tv_action_share).setOnClickListener {
                Toast.makeText(BaseApplication.mContext, "分享", Toast.LENGTH_SHORT).show()
            }
            getView<TextView>(R.id.tv_action_reply).setOnClickListener {
                Toast.makeText(BaseApplication.mContext, "评论", Toast.LENGTH_SHORT).show()
            }
        }
    }
